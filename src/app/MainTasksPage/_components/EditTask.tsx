...
export default function EditTask({ editID }: AddTaskType) {
  const { editTask } = useTasksStore();
  const { taskName, setTaskName, taskDescription, setTaskDescription } = useInputsStore();
  const { isSubmitting, setIsSubmitting, isOpenEditDialog, setOpenEditDialog } = useDialogInfoStore();
  ...
  // Functions
  const handleSubmitEditDialog = async (e: React.FormEvent) => {
    e.preventDefault()
    setIsSubmitting(true)
    // Simulating an API call
    await new Promise(resolve => setTimeout(resolve, 400));
    editTask(editID, taskName, taskDescription);
      ...
      {isOpenEditDialog && (
        <div className="fixed inset-0 z-10 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75 backdrop-blur-sm" aria-hidden="true" onClick={closeEditDialog}></div>
              ...
              <div className="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="w-full mt-3 text-center sm:mt-0 sm:text-left">
                    ...
                    <div className="mt-4">
                      <form onSubmit={handleSubmitEditDialog} className="space-y-6">
                        <div>
                          ...
                          <input
                            ...
                            className={`${theme === 'dark' ? 'text-white' : 'text-gray-700'} w-full text-sm px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400 transition-colors duration-200`}
                        ...
                        <div className="mt-8 sm:flex sm:flex-row-reverse">
                          <button
                            onClick={() => {
                              showAlertBtn('The mission has been modified')
                            }}
                            type="submit"
